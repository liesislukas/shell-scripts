...
fn_set_os() {
  ...
  if command -V lsb_release >/dev/null 2>&1; then
    ...
    if ! ls /etc/*-release >/dev/null 2>&1; then
      ...
      if [ -z "$os" ]; then
        if
          grep -i "oracle linux" /etc/*-release >/dev/null 2>&1 || \
          grep -i "red hat" /etc/*-release >/dev/null 2>&1
        then
          os="rhel"
        else
    ...
    centos)
      ...
      os_release=$(
        cat /etc/*-release | grep -i 'centos.*[0-9]' | \
        sed 's/^[^0-9]*\([0-9][0-9]*\).*$/\1/' | head -1
      )
      ;;
    rhel | ol)
      os_codename=$(
        cat /etc/*-release | grep -i 'red hat.*(' | \
        sed 's/.*(\(.*\)).*/\1/' | head -1 | \
        tr '[:upper:]' '[:lower:]'
      )
      # For Red Hat also grab release
      os_release=$(
        cat /etc/*-release | grep -i 'red hat.*[0-9]' | \
        sed 's/^[^0-9]*\([0-9][0-9]*\).*$/\1/' | head -1
      )
    ...
    amzn)
      ...
      release_amzn=$(
        cat /etc/*-release | grep -i 'amazon.*[0-9]' | \
